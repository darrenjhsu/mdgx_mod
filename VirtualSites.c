...
static void InsertExtraPoint(prmtop *tp, int resid, int ruleid)
{
  ...
  tp->AtomNames = ShivForChar4(tp->AtomNames, tp->natom-1, atmpos,
                               ert->epname);
  tp->AtomTypes = ShivForChar4(tp->AtomTypes, tp->natom-1, atmpos, "LP  ");
...
//                 Amber conventions.  This means pretty much anything other
//                 than EPs on carbonyl oxygen.  If there is one point, it will
//                 lie in the plane of the three frame along the line defined
                ...
                double *dloc, double *eploc, double *epgloc, expt *tmr)
{
  ...
  double rab[3], rac[3], rbc[3], rbd[3], abac[3], rPerp[3];
		  ...
		  double *dfrc, double *epfrc, expt *tmr)
{
  ...
  double dotuv, sfac, uvdis, vudis, dphidu, dphidv, dphidw;
  double rab[3], rac[3], rbc[3], fb[3], fc[3], raEP[3], rPerp[3], pbold[3];
  double dr[3], up[3], vp[3];
